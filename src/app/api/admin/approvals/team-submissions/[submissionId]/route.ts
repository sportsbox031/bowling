...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  const submissionRef = adminDb.doc(firestorePaths.teamEntrySubmission(tournamentId, ctx.params.submissionId));
  ...
  const projection = await applyApprovedTeamEntrySubmission(adminDb, submission, {
    approvedAt: now,
  ...
  return NextResponse.json({
    ok: true,
    status: "APPROVED",
    projectedTeamIds: projection.teamIds,
